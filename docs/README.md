...
| [decisions/2026-09-21-column-widths-are-measured-not-declared.md](decisions/2026-09-21-column-widths-are-measured-not-declared.md) | Why a sized column in the commit list measures what it draws rather than carrying a fixed width, what a stored width means and why only the columns the user dragged are restored, why the pref file is treated as untrusted, why auto-fit measures only the pages that have loaded, and how the header and the rows read one width per column from a custom property on the list's root. Read before changing column widths, their persistence, the auto-fit effects, or how a cell gets its width. |

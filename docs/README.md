...
| [architecture/scrollbars.md](architecture/scrollbars.md) | Why every native scrollbar is hidden and a themed thumb is painted from JS instead, why the thumb shows only while scrolling, the second thumb a pane gets while it scrolls sideways, the thumb a component announces for a pan it keeps itself (the Graph pan), how dragging works, and the padding and overlay traps that make lists which fit still scroll. Read before touching `scrollbar-activity.ts`, the `::-webkit-scrollbar` rule, or `VirtualList`'s height measurement. |

...
| [decisions/2026-09-01-how-a-renamed-file-reads-on-one-row.md](decisions/2026-09-01-how-a-renamed-file-reads-on-one-row.md) | How a rename is written on a one-line file row: `old.ts → new/path.ts`, dropping the directory when the file did not move, keeping both paths whole when it did. The rejected options and the evidence against them, including why git's own `{a => b}` braces are the worst shape under a right-truncating ellipsis. Read before changing how a renamed row is written. |

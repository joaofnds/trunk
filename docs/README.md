...
| Doc | What it covers |
|-----|----------------|
| [../GLOSSARY.md](../GLOSSARY.md) | The project's domain terms in João's words: lanes and claims in the graph, the graph test corpus, dates, commit navigation, the review model, remote operations, the application harness, and time in the frontend. Read it first; add a term when you learn one. |
...
| [architecture/scrollbars.md](architecture/scrollbars.md) | Why every native scrollbar is hidden and a themed thumb is painted from JS instead, why the thumb shows only while scrolling, the second thumb a pane gets while it scrolls sideways, the thumb a component announces for a pan it keeps itself (the Graph and Message pans), which wheel events of a swipe a pan takes and why, how dragging works, and the padding and overlay traps that make lists which fit still scroll. Read before touching `scrollbar-activity.ts`, the `::-webkit-scrollbar` rule, `VirtualList`'s height measurement, or the commit list's wheel handler. |
...
| [decisions/2026-09-21-column-widths-are-measured-not-declared.md](decisions/2026-09-21-column-widths-are-measured-not-declared.md) | Why a sized column in the commit list measures what it draws rather than carrying a fixed width, why each fit stops at a cap and the fits share the list's width in a set yield order, why a user width has a floor and no ceiling, what a stored width means and why only the columns the user dragged are restored, why the pref file is treated as untrusted, when fits are measured and the budget re-applied, how the header and the rows read one width per column from a custom property on the list's root, and why, past the list's width, the table scrolls sideways under a header that mirrors it, with a swipe over the Graph column or Message panning that column to its end before it moves the table. Read before changing column widths, their persistence, the fit effect, how a cell gets its width, or how a sideways swipe pans the Graph or Message column. |
...
| Doc | What it covers |
|-----|----------------|
| [accessibility/contrast-re-audit-2026-06-22.md](accessibility/contrast-re-audit-2026-06-22.md) | The 2026-06-22 pass — every surface re-derived against live source, with the fixes that landed. Its word-stack numbers describe a state the 2026-09-05 decision replaced; the gate is the current record. |
...
| [build-environment.md](build-environment.md) | What `just check`'s and `just dev`'s speed depends on beyond the code: the single pinned toolchain shared by every session, why switching between `just dev`, `just dev-app` and `just check` must not rebuild dependencies and how to ask cargo why one did, the macOS Gatekeeper stall that turns minute gates into hour gates and how to diagnose it, and why no scanner may walk `src-tauri/target`. Also why `just dev` is invisible to an agent's screen tools and `just dev-app` is the route that works. Read when the gate is slow with idle CPUs, when a build rebuilds dependencies no edit touched, or before trying to drive the running app. |
| [release.md](release.md) | `just release <version>`: what it bumps (`package.json`, `src-tauri/Cargo.toml`, `src-tauri/Cargo.lock`, `src-tauri/tauri.conf.json`), its dirty-tree and non-increment guards, how it recovers from a run that committed but never tagged, and the pure/thin split between `scripts/release.ts` and the justfile recipe. Read before cutting a release or changing the recipe. |
...
| [decisions/2026-09-02-ring-tells-a-busy-subscriber-from-a-dead-one.md](decisions/2026-09-02-ring-tells-a-busy-subscriber-from-a-dead-one.md) | Why the reviewdb doorbell sweeper decides a subscriber is dead from the pid in its socket name rather than from the `connect` error: a live listener with a full backlog and a dead one both answer `ConnectionRefused`, so unlinking on the error deafened live subscribers permanently. Read before changing `ring` or the socket naming in `reviewdb::events`. |
...
| [visual-regression.md](visual-regression.md) | `just visual`: the real app screenshotted in WebKit against the graph fixture repositories and compared with committed baselines within 24 colour levels a channel. How to run it, read a difference image, and accept a change; the measured wall times and the alternatives tried; what it does not cover. Read before touching `tests/visual/` or when a capture differs. |
| [visual-baseline-changelog.md](visual-baseline-changelog.md) | Every accepted change to a visual baseline, with the reason. Written only by `just visual-accept "<reason>"`. |
| [decisions/2026-09-26-visual-regression-baselines.md](decisions/2026-09-26-visual-regression-baselines.md) | Why the graph pane is captured per fixture repository and once at a narrowed width, why native macOS WebKit through the real app and `app_host`, why a 24-level channel tolerance (measured against GitHub's macOS runner) and not Playwright's comparator, and why vitest rather than `@playwright/test`. |

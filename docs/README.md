...
| [decisions/2026-09-01-how-a-renamed-file-reads-on-one-row.md](decisions/2026-09-01-how-a-renamed-file-reads-on-one-row.md) | How a rename is written on a one-line file row: `code/{util.ts → math-util.ts}`, naming a shared directory once and scoping both names under it, or both paths whole when the file moved. Why the earlier `util.ts → code/math-util.ts` was misleading, and why git's dangling-brace truncation does not transfer to a row of three spans. Read before changing how a renamed row is written. |

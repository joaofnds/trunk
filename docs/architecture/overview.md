...
| `commands/diff.rs` | `diff_unstaged`, `diff_staged`, `diff_commit`, `diff_commit_file`, `list_commit_files`, `get_commit_detail` — highlighting parses a bounded window above each side's first displayed line, with no cache and no warming | `src-tauri/src/commands/diff.rs` |

...
the Branch/Tag column now fits its widest ref instead of a fixed 120px default, so the lanes start at the column's real width; and restoring persisted widths no longer overwrites the graph auto-fit, so a graph whose lanes fit its column stops drawing the edge-fade mask that said they ran past it
...
Changed goldens:
     ...
     M src/__tests__/goldens/graph-render/lane-03-detached-old.txt
     M src/__tests__/goldens/graph-render/lane-04-tiebreak-upstream-vs-topic.txt
     M src/__tests__/goldens/graph-render/lane-05-diverged.hidden.txt
     M src/__tests__/goldens/graph-render/lane-05-diverged.txt
     M src/__tests__/goldens/graph-render/lane-06-tag-only-chain.txt
     M src/__tests__/goldens/graph-render/lane-07-tag-on-unpulled.txt
     M src/__tests__/goldens/graph-render/lane-08-stash-on-tip-behind.txt
...
the Branch/Tag column now sizes to the pill each row actually draws, the highest-priority ref plus its +N badge, so a label that shared its row with a badge is no longer truncated by the badge's width
...
TRUNK-255: scope the graph's clip-path ids per mounted instance so one tab's collapsed column stops clipping another's rails; clip the dot layer so a collapsed column stops painting dots over the message text; fade the rails out under the hugged dots instead of cutting them at a hard line, hiding them entirely once the hug covers the band; clamp dots to lane centres; and inset every stroked marker by half its stroke so the merge ring, WIP circle and stash square measure the same 12px as a solid dot. Joao tested the rendered result in the app and directed the accept.
...
each ref pill is clipped to the Branch/Tag column so nothing it draws paints over the lanes, and the +N badge is reserved at the 26px it is drawn rather than 20px, so lane-10's label reads origin/… where it read origin/m…
...
restoring stored widths no longer overwrites the fits, so the graph column keeps its lane fit instead of the 24px default and 39 goldens stop drawing rail fades that said the lanes ran past the column
...
the Branch/Tag column fits the widest pill its page shows instead of a fixed 120px, so every lane offset leaves 124: 57 narrower, and lane-10 wider at 138 with origin/main whole beside its badge

...
all fifteen in `graph_input.rs`; §"Extraction-introduced sites" describes them by group.
`scripts/graph-mutation-sweep.py` carries all 64.

**Run 2026-08-12** against the suite as it stands after the capture migration. Regenerate with
`just graph-sweep`; the table below is that command's stdout, pasted unedited.
...
**Coverage: 64 of the pipeline's 139 mutants.** 36 of `placement.rs`'s 111 (Appendix A of the
milestone-5 grill), all 13 of `graph.rs`, and all 15 of `graph_input.rs`. The remaining 75 are
...
at 12-space indent that is not an anchor, and editing that phase-1 rule leaves all 64 anchors
...
enumerated with `cargo mutants --list` and anchored by hand.

**35 killed, 17 survive, 12 unviable.** Every one of the 17 survivors carries a construction
...
| 11 | placement.rs | can_inline off-chain disjunct | `\|p\| !head_chain.contains(&p) \|\| input.head_tip == Some(p)` → `\|p\| head_chain.contains(&p) \|\| input.head_tip == Some(p)` | killed | 2026-08-12 |
| 12 | placement.rs | inline branch guard | `                if c >= active_lanes.len() {` → `                if c < active_lanes.len() {` | killed | 2026-08-12 |
| 13 | placement.rs | inline branch resize | `active_lanes.resize(c + 1, None);` → `active_lanes.resize(c - 1, None);` | SURVIVES | 2026-08-12 |
| 14 | placement.rs | inline branch resize | `active_lanes.resize(c + 1, None);` → `active_lanes.resize(c * 1, None);` | SURVIVES | 2026-08-12 |
...
other twenty end killed. Rows 38 to 64 produced no survivor, so no proof below covers them. No
row is left open.

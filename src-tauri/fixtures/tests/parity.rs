...
/// Doc-45 §3: commits on two branches, a --no-ff merge, an octopus of two topics, an
/// annotated and a lightweight tag, plus a split-date commit and a branch at a revspec,
/// and case 05-01's octopus of three topics behind an advanced main.
...
fn the_repo_verbs_write_the_bytes_git_writes() {
    ...
    let mut git = GitCli::init(&root.path().join("git"), "main", FIXTURE);
    let mut repo = Repo::init(&root.path().join("repo"), "main", FIXTURE);

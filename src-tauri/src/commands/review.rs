...
fn open_cached(
    ...
    let store = match slot.as_ref() {
        ...
        None => Arc::clone(slot.insert(Arc::new(trunk_review::reviewdb::open(data_dir)?))),
    };
    drop(slot);
...
pub struct SubmitThreadRequest {
    pub text: String,
    pub anchor: Option<trunk_review::types::Anchor>,
    ...
    pub content_pin: Option<trunk_review::types::ContentPin>,
...
pub fn set_thread_state_inner(
    ...
    canonical: &Path,
    id: &str,
    next: trunk_review::types::ThreadState,
    now: i64,
) -> Result<(), TrunkError> {
    store.write(|tx| {
        threads::set_state(
...
fn as_comments(threads: Vec<threads::Thread>) -> Vec<trunk_review::types::Comment> {
    threads
        .into_iter()
        .map(|t| trunk_review::types::Comment {
            id: t.id,
            text: t.text,
            anchor: t.anchor,
            cached_excerpt: t.cached_excerpt,

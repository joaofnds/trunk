...
pub fn configure<R: tauri::Runtime>(
    ...
    let builder = builder
        ...
        .register_uri_scheme_protocol("trunk-asset", serve_trunk_asset)
    ...
    register_state(builder, watcher, traffic_lights).invoke_handler(tauri::generate_handler![
...
/// Serve a `trunk-asset://` request from the repository the URL names.
fn serve_trunk_asset<R: tauri::Runtime>(
...
/// Build the app menu, start the review-store poll, and place the traffic lights.
...
/// The application, edit, view and window menus.
...
/// Announce foreign store commits — a CLI reply, another instance — so they
/// appear without a restart.
///
/// The payload-free emit means "refresh yours"; the app's own writes also land
/// here and cause a benign self-refetch, the same shape as the per-repo emits.
fn start_review_poll<R: tauri::Runtime>(app: &tauri::App<R>) {
...
/// Register every piece of state a command can be injected with.

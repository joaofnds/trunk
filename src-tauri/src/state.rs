...
/// the async runtime.
#[derive(Default)]
pub struct StoreSlot(std::sync::Arc<Mutex<Option<std::sync::Arc<trunk_review::reviewdb::Store>>>>);

impl StoreSlot {
    #[must_use]
    pub fn clone_handle(
        &self,
    ) -> std::sync::Arc<Mutex<Option<std::sync::Arc<trunk_review::reviewdb::Store>>>> {

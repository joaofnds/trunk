...
//! document without highlighting its code fences (L-10), so it may not depend on a
//! highlighter. The compiler refuses a `use` of any of these only while the crate's
//! manifest does not declare it, and this suite is what stops the manifest from
//! declaring it.
use std::path::Path;
use std::process::Command;

/// The packages `package` declares as normal or build dependencies on any target.

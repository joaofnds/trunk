...
use trunk_lib::types::LinePairing;
use trunk_lib::types::{DiffOrigin, DiffRequestOptions, DiffStatus};
...
fn diff_unstaged_ignores_whitespace_when_enabled() {
    ...
    let ignore_lines: usize = result_ignore
        ...
        .filter(|l| {
            matches!(
                l.origin,
                trunk_lib::types::DiffOrigin::Add | trunk_lib::types::DiffOrigin::Delete
            )
        })
        .count();
    assert_eq!(
...
fn ambiguous_slider_lands_on_the_boundary_git_cli_picks() {
    ...
    let added: Vec<&str> = file_diffs[0]
        ...
        .iter()
        .flat_map(|h| h.lines.iter())
        .filter(|l| matches!(l.origin, trunk_lib::types::DiffOrigin::Add))
...
fn reflowed_paragraph_lines_carry_their_partner() {
    ...
    let pairings: Vec<_> = file_diffs[0].hunks[0]
        .lines
        .iter()
        .filter(|l| !matches!(l.origin, trunk_lib::types::DiffOrigin::Context))
        .map(|l| l.pairing)
        .collect();
...
fn word_span_basic_pair() {
    ...
    let del_line = hunk
        .lines
        .iter()
        .find(|l| matches!(l.origin, trunk_lib::types::DiffOrigin::Delete))
        .expect("expected a Delete line");
    let add_line = hunk
        .lines
        .iter()
        .find(|l| matches!(l.origin, trunk_lib::types::DiffOrigin::Add))
...
fn word_span_long_line_skipped() {
    ...
    for line in &hunk.lines {
        if matches!(
            line.origin,
            trunk_lib::types::DiffOrigin::Delete | trunk_lib::types::DiffOrigin::Add
        ) {
            assert!(
...
fn unstaged_diff_highlights_an_untracked_rust_file_with_no_old_side() {
    ...
    assert!(
        hunk.lines
            .iter()
            .all(|l| !matches!(l.origin, trunk_lib::types::DiffOrigin::Delete)),
...
fn compare_across_divergent_branches_lists_tree_differences() {
    ...
    assert_eq!(a.status, trunk_lib::types::DiffStatus::Modified);
    let f = files.iter().find(|f| f.path == "f.txt").unwrap();
    assert_eq!(f.status, trunk_lib::types::DiffStatus::Added);
}
...
fn compare_file_diff_direction_is_base_to_target() {
    ...
    let deleted: Vec<&str> = hunk
        .lines
        .iter()
        .filter(|l| matches!(l.origin, trunk_lib::types::DiffOrigin::Delete))
...
fn compare_with_no_base_diffs_against_the_empty_tree() {
    ...
    assert_eq!(files[0].status, trunk_lib::types::DiffStatus::Added);
}

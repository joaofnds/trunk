...
use trunk_review::snapshot::{SnapshotKind, pinned_snapshot_oids};
...
fn the_sweep_reclaims_superseded_pins_nothing_anchors_to() {
    ...
    let refs: Vec<String> = repo
        .references_glob(&format!("{}*", trunk_review::snapshot::SNAPSHOT_REF_PREFIX))
        .unwrap()
        .filter_map(std::result::Result::ok)
    ...
    assert_eq!(
        ...
        vec![format!(
            "{}{}",
            trunk_review::snapshot::SNAPSHOT_REF_PREFIX,
            last_oid
        )],
...
fn a_pin_survives_supersession_while_a_thread_anchors_to_it() {
    ...
    let prefix = trunk_review::snapshot::SNAPSHOT_REF_PREFIX;
    assert!(
...
fn review_deletion_touches_no_refs() {
    ...
    let snap_ref = format!("{}{}", trunk_review::snapshot::SNAPSHOT_REF_PREFIX, oid);
    assert!(
...
fn a_record_with_no_ref_is_dropped() {
    ...
    trunk_review::snapshot::prune_snapshot_ref(&repo, git2::Oid::from_str(&oid).unwrap()).unwrap();
...
fn the_decision_sees_the_reconciled_record() {
    ...
    trunk_review::snapshot::prune_snapshot_ref(&repo, git2::Oid::from_str(&vanished).unwrap())
        .unwrap();

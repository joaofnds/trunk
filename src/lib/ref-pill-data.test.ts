...
function makeCommit(
	overrides: Partial<GraphCommit> & { refs?: RefLabel[] },
): GraphCommit {
	return {
		...
		lane_ref: overrides.lane_ref ?? null,
	};
}
...
describe("buildRefPillData", () => {
	...
	it("skips WIP nodes", () => {
		...
		const commits = [
			makeCommit({ refs: [makeRef({ short_name: "main", is_head: true })] }),
		];
		const result = buildRefPillData(nodes, commits, 200, mockMeasure);
		expect(result).toEqual([]);
	});
	...
	it("isRemoteOnly=true when ref is RemoteBranch AND no sibling LocalBranch or Tag", () => {
		const refs = [
			makeRef({ short_name: "origin/feature", ref_type: "RemoteBranch" }),
		];
		const nodes = [makeNode({ x: 0, y: 0 })];
		const commits = [makeCommit({ refs })];
		const result = buildRefPillData(nodes, commits, 200, mockMeasure);
	...
	it("pill width = textWidth + PILL_PADDING_X*2 + ICON_WIDTH + ICON_GAP for branches", () => {
		...
		expect(result[0].width).toBe(
			21 + PILL_PADDING_X * 2 + ICON_WIDTH + ICON_GAP,
		);
	});
	...
	it("pill width includes ICON_WIDTH + ICON_GAP for Tag refs", () => {
		...
		expect(result[0].width).toBe(
			28 + ICON_WIDTH + ICON_GAP + PILL_PADDING_X * 2,
		);
	});
